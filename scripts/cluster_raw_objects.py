...
def cluster_raw_objects(data_path, visualize):
    ...
    for s in sweeps:
        print s
        if visualize:
            subprocess.call(['rosrun', 'rbpf_processing', 'consolidate_raw_detections', s, '--visualize'])
        else:
            subprocess.call(['rosrun', 'rbpf_processing', 'consolidate_raw_detections', s])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        cluster_raw_objects(sys.argv[1], False)
    elif sys.argv[2] == "--visualize":
        cluster_raw_objects(sys.argv[1], True)
    else:
        print "Usage: ", sys.argv[0], " path/to/data (--visualize)"
